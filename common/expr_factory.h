...
class ExprFactory {
 ...
 public:
  ...
  Expr NewBind(NextIdFunc next_id, BindVar bind_var,
                        BindExpr bind_expr, RestExpr rest_expr) {
    Expr expr;
    expr.set_id(next_id());
    auto& comprehension_expr = expr.mutable_comprehension_expr();
    comprehension_expr.set_iter_var("#unused");
    comprehension_expr.set_iter_range(NewList(next_id(), std::vector<cel::ListExprElement>{}));
